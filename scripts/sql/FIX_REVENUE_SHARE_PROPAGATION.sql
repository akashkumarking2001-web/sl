-- ==============================================================================
-- FIX: REVENUE SHARE COUNT PROPAGATION (RECURSIVE ANCESTOR UPDATE)
-- ==============================================================================
-- Previous logic only updated the direct sponsor's count. 
-- This updated function propagates the "+1" count all the way up to the Root Node.
-- It also checks income milestones for EVERY ancestor along the way.
-- ==============================================================================
...
DECLARE
    ...
    v_current_node_user_id UUID;
    v_ancestor_found BOOLEAN;
BEGIN
    -- 1. Fetch Settings
    ...
    -- ====================================================
    -- D. REVENUE SHARE TREE (3xN Matrix Placement & Propagation)
    -- ====================================================
    ...
    IF v_buyer_profile.sponsor_id IS NOT NULL OR v_buyer_profile.referred_by IS NOT NULL THEN
        ...
        IF FOUND THEN
            ...
            SELECT id, user_id INTO v_target_node_id, v_target_user_id
            FROM bfs
            ...
            IF v_target_node_id IS NOT NULL THEN
                ...
                IF v_pos IS NOT NULL THEN
                    ...
                    -- A. LINK NODE
                    EXECUTE format('UPDATE public.revenue_share_tree SET %I = $1 WHERE id = $2', v_pos)
                    USING p_buyer_id, v_target_node_id;

                    -- B. RECURSIVE UPDATE: Propagate Count UP the tree
                    v_current_node_id := v_target_node_id; -- Start with the new parent
                    v_current_node_user_id := v_target_user_id;
                    ...
                    LOOP
                        ...
                        RETURNING downline_count INTO v_new_downline_count;

                        -- 2. Check Milestones for this Ancestor
                        v_amount := 0; v_level := 0;
                        ...
                        -- 3. Move UP to Parent
                        -- We look for the node that has 'v_current_node_user_id' as a child (left/mid/right)
                        v_ancestor_found := false;
                        
                        SELECT id, user_id INTO v_current_node_id, v_current_node_user_id
                        FROM public.revenue_share_tree
                        WHERE (left_pos = v_current_node_user_id OR mid_pos = v_current_node_user_id OR right_pos = v_current_node_user_id)
                        AND package_type = p_package_name
                        LIMIT 1;
                        
                        IF FOUND THEN
                            v_ancestor_found := true;
                        ELSE
                            EXIT; -- Reached Root or broken chain
                        END IF;
            ...
            ELSE
                 ...
                 -- This handles the very first placement if the BFS returned nothing
                 ...
                 SET left_pos = EXCLUDED.left_pos; 
            END IF;
        END IF;
    END IF;
